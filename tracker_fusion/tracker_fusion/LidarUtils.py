...
def compute_box_3d(l2c_object, obj):
    ...
    # compute object position in camera coordinates
    obj_pos = np.dot(l2c_object.V2C, np.append(obj.t, 1))

    xl = obj_pos[0]
    yl = obj_pos[1]
    zl = obj_pos[2]

    #print(f'l: {l} w: {w} h: {h} xl: {xl} yl: {yl} zl: {zl}')
    ...
    #pointsloc = [xl, yl, zl, 1]
    #pointslocnp = np.array(pointsloc)
    #print("nube de pointsloc: ", np.dot(l2c_object.V2C, np.transpose(pointslocnp)))
    #print("tamaño pointsloc: ", pointslocnp.shape)
    ...
    # perform translation in all axes
    corners_3d[0, :] = corners_3d[0, :] + xl 
    corners_3d[1, :] = corners_3d[1, :] + yl + 1.3
    corners_3d[2, :] = corners_3d[2, :] + zl 
...
def convert_3D_to_2D(l2c_object, pts_3d_velo, print_values=False):
    #print("nube de puntos: ", pts_3d_velo)
    #print("tamaño puntos: ", pts_3d_velo.shape)
    '''
    ...
    return pts_2d
def convert_3D_to_3D(l2c_object, pts_3d_velo, print_values=False):
    '''
    Input: 3D points in Velodyne Frame [nx3]
    Output: 3D Points in Camera Frame [nx3]
    '''
    R0_homo = np.vstack((l2c_object.R0, [0, 0, 0]))
    if print_values:
        print("R0_homo ", R0_homo)
    ...
    pts_3d_camera = np.dot(P_R0_Rt, pts_3d_homo.T)
    if print_values:
        print("pts_3d_camera ", pts_3d_camera)

    return pts_3d_camera.T[:, :3]
    


def remove_lidar_points_beyond_img(l2c_object, pts_3d, xmin, ymin, xmax, ymax, clip_distance=0.1):
    #print(xmax)
    #print(ymax)
...
def get_lidar_on_image(l2c_object, pc_velo, size):
    ...
    imgfov_pc_velo, all_pts_2d, fov_inds = remove_lidar_points_beyond_img(l2c_object,
        pc_velo, 0, 0, size[0], size[1], 0.05
    )
...
def display_lidar_on_image(l2c_object, pc_velo, image):
    ...
    for i in range(pts_2d.shape[0]):
        ...
        # Ajustar el rango de profundidad entre 0 y 2
        depth = max(min(depth, 2), 0)

        # Actualizar el cálculo del color
        color = cmap[int(255.0 * depth / 2), :]
